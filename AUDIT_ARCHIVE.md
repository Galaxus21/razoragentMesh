# Audit archive — the 52 closed findings

Split out of `AUDIT_TODO.md` on 2026-09-04, so that file holds only work that is still open.
Item 52 was closed the same day and moved here.
The findings are unedited: the severity grouping, the scope notes and each finding's evidence
stand as they were written, followed by the note recording what closed it. Only this header is
new, replacing the old file's title. **Item numbers are never
reused** -- code comments and `rehearsalFixPlan.md` cite findings by number
(`healingTelemetry.py:72` cites item 50), so those references resolve here.

Findings 1-30 came from the read-only survey of 2026-09-01 and are grouped by severity; 31 onward
surfaced during remediation and in the live runs after it, in the order they were found. One
finding, **48**, is still open and stayed behind in [AUDIT_TODO.md](AUDIT_TODO.md).

Every finding was verified against the source and carries the `file:line` that proves it; nothing
here is inferred from docs. Within each severity section, findings are ordered by what would have
cost most if a judge had found it first.
...
### [x] 51. The payout destination was caller-supplied and bound to nothing signed

Found on 2026-09-04 while writing the merchant-facing half of `telemetry.mdx`, and fixed the same
day. `execute_settlement` took a `merchant_account` argument and it won over the account bound at
cart creation:

    merchantAccount: request.merchant_account ?? session.merchantAccount ?? defaultMerchantAccount
    -- packages/mcpServer/src/tools/settlementExecutor.ts, _postSettlement (before the fix)

Nothing signed constrained that value. The `CartMandate` a merchant signs carries `merchantDid`
and **no payout account** -- the signature attests to the prices and to who is selling, and says
nothing about where the money goes -- and no code path looked up the `razorpayAccountId` the
merchant gave at registration. Any string matching `acc_[A-Za-z0-9_]+` was accepted. So the field
deciding where money lands was the one field on the request a buyer agent chose freely.

The tool description already claimed otherwise: it told agents `merchant_account` "Must match the
cart mandate". That is the stale-description failure inverted -- the usual case is a description
describing a guard that changed, and this described a guard never written, which is worse, because
a reader of the manifest comes away believing the check exists.

**Why it mattered:** harmless while Route transfers run on the deterministic ledger -- with
`RAZORPAY_ROUTE_LIVE` unset, `routeClientFactory` picks the mock transport and a "transfer" is an
arithmetic record. It would have become a live funds-redirection path on the day Route is
activated with real linked accounts, which is the day the demo becomes a product.

**Fixed** by `packages/mcpServer/src/merchant/merchantPayoutRegistry.ts`, which resolves the
destination from the merchant identity the cart was *signed* with and admits exactly two sources:
the mesh's own merchant key, derived from `MERCHANT_PRIVATE_KEY_HEX` so re-keying moves the DID
and the binding follows it; and a `MerchantProfile` registered through the Merchant API, read from
the Redis key `POST /api/v1/merchant/register` writes. An unknown merchant is refused rather than
given an invented destination, and a profile whose account id is not a Route account id is refused
rather than passed on. `settlementExecutor` now resolves from `session.cartMandate.merchantDid` and
calls `assertRequestedMerchantAccountMatches` before anything reaches the engine, so a caller can
only agree with the resolution or be refused -- and `_postSettlement` takes the account as a
parameter rather than a request field, so a future edit cannot reintroduce the override. The same
change rewrote both manifest descriptions: `create_cart_mandate` now says the account "is not a
destination you can choose. Omit it", and `execute_settlement` says it is "registered to the
merchantDid on the signed cart; no field on this call can move it".

Eleven tests in `packages/mcpServer/test/merchantPayoutBinding.test.ts` cover it, including the two
that matter most: a redirected `merchant_account` is refused **without reaching the engine**, so
nothing is charged, and a caller that passes the correctly resolved account back is *not* refused
-- agreement must not read as an attack.

### [x] 53. `GUIDE.md` documented five dashboard routes that no longer resolve

§6.2 was titled "Deep Dive into the 7 Dashboard Routes" and gave a sitemap table plus a subsection
each for `/agent-observability`, `/negotiation-hub`, `/security-audit`, `/self-healing` and
`/infrastructure`. Those five pages were consolidated into `/visualise` -- the reason is recorded
in `sidebarNavigationConfig.ts`: five panels fed by one SSE array meant watching a purchase took
five pages, and no single screen ever showed the run happening. `find src/app -name page.tsx`
returns eight routes and none of the five.

Two of the stale URLs were in the **pitch script** (§8.1), which told the presenter to navigate to
`/self-healing`, `/security-audit` and `/infrastructure` on stage.

Fixed: §6.2 now documents the four sidebar categories over eight routes as they ship, §6.1's "7
Lucide navigation items" is corrected, the two pitch-script lines point at `/visualise`, and the
new `/visualise/vectors` tab is documented with the figures it was measured showing.

### [x] 52. `webhookVerifier` is tested, exported, and mounted on no route

`packages/mandateEngine/settlement/webhookVerifier.py` implements HMAC-SHA256 signature
verification with `hmac.compare_digest` and a 300-second freshness window bounded in both
directions (`webhookFreshnessWindowSeconds`, whose comment explains that a captured webhook stays
signature-valid forever). `tests/testWebhookVerifier.py` covers it and passes.

No route calls it. `mandateApp.createMandateApp` mounts exactly four endpoints -- health, settle,
`GET /api/v1/telemetry/stream`, `POST /api/v1/telemetry/events` -- plus `registerCheckoutRoutes`,
and none is a webhook receiver. Outside the tests, the only references to the three functions are
the re-exports in `settlement/__init__.py` and `mandateEngine/__init__.py`. Grepping the other
services finds webhook *handling* only in `merchantApi/src/routes/bulkIngestRoute.py` (Shopify
product sync, a different protocol with no Razorpay signature) and webhook *sending* only in
`x402Gateway/src/alerts/priceDropAlertManager.py` (price-drop alerts to a buyer's callback URL).

**Why it matters:** it reads as an implemented capability from the outside -- the module exists and
its tests are green -- which is item 23's shape exactly (`X402ChallengeMiddleware`, written and
never mounted). Concretely: payment state comes only from the synchronous settle call, so nothing
can tell the mesh that a payment later failed, was disputed or was refunded out of band. Nor is
there any egress in the other direction: no email, SMS or merchant callback fires when a sale
settles. Until the *Merchant-side subscribers* section of `telemetry.mdx` was written, no document
said so, and a merchant reading the guides would reasonably have assumed a notification exists.

**Fixed 2026-09-04.** `POST /api/v1/webhooks/razorpay` is mounted by
`mandateEngine/webhooks/razorpayWebhookRoute.py`, registered from `createMandateApp` beside the
checkout routes. It calls the verifier that was already written: HMAC-SHA256 with
`hmac.compare_digest`, and the 300-second freshness window enforced by passing the
`X-Razorpay-Event-Time` header through to `verifyRazorpayWebhookSignature`. Four distinct answers,
because conflating them turns a misconfiguration into a suspected attack -- **503** when
`RAZORPAY_WEBHOOK_SECRET` is unset (fail closed: an endpoint that 200s what it cannot verify is
worse than no endpoint), **401** on a bad signature, **400** on a stale delivery, and **200
`duplicate`** on a retry of an event id already seen, de-duplicated through Redis `SET NX` with a
24-hour TTL. The duplicate answer is 200 on purpose: any non-2xx tells Razorpay to retry, which is
exactly what that branch exists to stop.

Seven tests in `tests/testRazorpayWebhookReceiver.py`, the first of which is
`testWebhookRouteIsActuallyMounted` -- the finding itself, asserted against `app.routes` so the
verifier cannot quietly become unreachable again. `RAZORPAY_WEBHOOK_SECRET` is in `.env.example`
and on the `mandate-engine` service in `docker-compose.yml`.

**What it deliberately does not do, and why that is not this item.** It reconciles nothing, and
says so in its own response body (`"reconciled": false`). `SettlementResult` is never persisted, so
a `payment.failed` or `refund.created` delivery has no order to amend -- that is the separate
order-persistence limitation recorded in `README.md`, and reconciliation is blocked on it rather
than on the receiver. The outbound half -- notifying a merchant that a sale settled -- is likewise
not a defect but an unbuilt feature, and stays in `README.md` under *Known limitations*, with the
SSE bus documented as the integration path in `telemetry.mdx` under *Merchant-side subscribers*.
