import { TelemetryEvent } from "../../src/types/telemetryEventTypes.js";

export function createTestNominalSettlementEvents(sessionId: string): ReadonlyArray<TelemetryEvent> {
  const now = Date.now();
  return [
    {
      eventId: `evt-pow-${now}`,
    ...
    },
    {
      ...
      payload: {
        ...
        callerAgentId: "did:agent:procurement-bot-01",
    ...
    },
    {
      ...
      payload: {
        ...
        signerKeyDid: "did:agent:user-cfo-01",
    ...
    },
    {
      ...
      payload: {
        ...
        signerKeyDid: "did:agent:procurement-bot-01",
...
export function createTestNegotiationScenarioEvents(sessionId: string): ReadonlyArray<TelemetryEvent> {
  const now = Date.now();
  return [
    {
...
export function createTestOosHealingScenarioEvents(sessionId: string): ReadonlyArray<TelemetryEvent> {
  const now = Date.now();
  return [
    {
