...
│   └── Anthropic JSON-RPC 2.0 Tools: search_catalog, get_live_sku_quote,               │
│       reserve_inventory_lock, verify_shipping_sla, establish_agent_delegation,         │
│       create_cart_mandate, sign_execution_mandate, execute_settlement                  │
...
| **INV-02** | Equal-Half GST Division | Intra-state: $\text{CGST} = \text{SGST} = \lfloor (\text{Taxable} \times \text{Rate}) / 200 \rfloor$ and $\text{IGST} = 0$. Inter-state: $\text{IGST} = \lfloor (\text{Taxable} \times \text{Rate}) / 100 \rfloor$ and $\text{CGST} = \text{SGST} = 0$. Total tax is *defined* as the sum of the levies. Both halves come from the identical expression, so they are equal by construction; deriving one as the remainder of the other would produce an illegal 2%/3% split on odd slabs such as 5%. | `arithmeticEnclave.py`, `pricingEngine.ts` |
...
│   ├── mcpServer/          # Model Context Protocol JSON-RPC Server (Port 4001 / Stdio)
...
│   └── x402Gateway/        # PoW Ingress & Negotiation State Machine (Port 4003)
...
# Ed25519 signing key for the merchant enclave. The buyer agent and the user principal are not
# configured here: both SDKs generate those keypairs at runtime with AgentKeyManager.generate(),
# which is what the examples in examples/ do.
