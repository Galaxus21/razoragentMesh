...
import { formatLatency } from "@/lib/currencyFormatter";
    ...
    <div className="flex h-full flex-col rounded-lg border border-borderSubtle bg-bgSurface p-4">
      <div className="flex items-center justify-between border-b border-borderSubtle pb-3">
        <div className="flex items-center gap-2">
          <Terminal className="h-4 w-4 text-statusInfo" />
          <h2 className="text-sm font-semibold text-textPrimary">Agent Thought & MCP Trace</h2>
          <span className="rounded bg-accentSubtle px-1.5 py-0.5 text-xs font-mono text-accentPrimary">
            {traceEvents.length}
          </span>
        </div>
        <div className="relative">
          <Search className="absolute left-2 top-2 h-3.5 w-3.5 text-textMuted" />
          <input
            type="text"
            placeholder="Filter traces..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="h-7 w-36 rounded-md border border-borderSubtle bg-bgSurface pl-7 pr-2 text-xs text-textPrimary placeholder:text-textMuted focus:border-accentPrimary focus:outline-none focus:ring-1 focus:ring-accentPrimary"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pt-3 space-y-2.5 pr-1 max-h-[380px] custom-scrollbar">
        {traceEvents.length === 0 ? (
          <div className="flex h-32 flex-col items-center justify-center text-xs text-textMuted">
            <Code2 className="mb-2 h-6 w-6 text-textMuted" />
          ...
          traceEvents.map((evt) => {
            ...
            return (
              <div
                key={evt.eventId}
                className={`rounded-md border bg-surfaceContainer p-2.5 transition ${style.borderColor} hover:bg-bgSurfaceHover`}
              >
                <div
                ...
                >
                  <div className="flex items-center gap-2">
                    {isExpanded ? (
                      ...
                      <ChevronRight className="h-3.5 w-3.5 text-textSecondary" />
                    )}
                    <span
                    ...
                    <span className="font-mono text-textPrimary font-medium truncate max-w-[140px] sm:max-w-[200px]">
                  ...
                  <div className="flex items-center gap-2">
                    {"durationMs" in evt.payload && (
                      <span className="flex items-center gap-1 font-mono text-xs text-statusInfo">
                ...
                {isExpanded && (
                  <div className="mt-2.5 border-t border-borderSubtle pt-2">
                    <pre className="max-h-48 overflow-x-auto rounded bg-bgBase p-2 font-mono text-xs text-textSecondary custom-scrollbar">
