...
def test_edge_case_06_gst_5_percent_intra_odd_penny_conservation() -> None:
    """EC-06: 5% Intra-state GST on 101 paise splits into equal CGST/SGST halves (2/2, total=4)."""
...
def test_edge_case_07_gst_5_percent_intra_parity() -> None:
    ...
    assert gst.totalTaxPaise == 1666
    assert gst.cgstPaise == 833
    assert gst.sgstPaise == 833
    assert gst.igstPaise == 0
...
def test_edge_case_08_gst_18_percent_inter_state() -> None:
    ...
    assert gst.totalTaxPaise == 13999
    assert gst.cgstPaise == 0
    assert gst.sgstPaise == 0
    assert gst.igstPaise == 13999
...
def test_edge_case_09_gst_0_25_percent_fractional_slab() -> None:
    ...
    assert gst.totalTaxPaise == 250
    assert gst.cgstPaise + gst.sgstPaise == 250
    assert gst.igstPaise == 0
...
def test_edge_case_17_calculate_route_splits_normal() -> None:
    ...
    assert split.commissionPaise == 200
    assert split.flatFeePaise == 50
    ...
    assert split.merchantNetPaise + split.totalFeePaise == 10000
...
def test_spending_cap_evaluation() -> None:
    ...
    assert r1.remainingDailyPaise == 20000

    r2 = evaluate_spending_cap(40000, 20000, 50000)
    assert r2.allowed is False
    assert "exceeds spending cap" in r2.violationReason

    r3 = evaluate_spending_cap(10000, 20000, 50000, single_tx_limit_paise=15000)
    assert r3.allowed is False
    assert "exceeds single transaction limit" in r3.violationReason
